...
def find_old_session_ids
  session_query =
    TestSession
      ...
      .join(
        SessionData.where(name: 'url', value: 'https://inferno.healthit.gov/reference-server/r4'),
        test_session_id: :id
      )
      .limit(5)

  session_query.map(:test_session_id)
end

def destroy_requests_and_headers(session_ids)
  ...
  request_ids.each_slice(10) do |ids|
    ...
    puts "Deleting #{requests_query.count} requests"
...
def destroy_messages_results_and_requests_results(session_ids)
  result_ids = Result.where(test_session_id: session_ids).map(:id)
...
loop do
  session_ids = find_old_session_ids

  break if session_ids.blank?

  puts "Starting to delete #{session_ids.length} sessions"
  destroy_requests_and_headers(session_ids)
  destroy_messages_results_and_requests_results(session_ids)
